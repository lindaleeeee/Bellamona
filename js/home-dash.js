...
function saveWeight(){const v=parseFloat(document.getElementById('wt-input')?.value);if(!v||v<20||v>300){alert('올바른 체중을 입력해주세요');return;}const now=new Date();const d=`${now.getMonth()+1}/${now.getDate()}`;S.weights.push({d,w:v});closeMod('mod-wt');updateHome();}
...
function renderDash(){
  ...
  const perDays=getPDays(y,m);const ovDays=getODays(y,m);const isCur=y===2026&&m===3;const goodDays=[7,8,12,13,19,20];
  ...
  for(let d=1;d<=total;d++){
    ...
    if(isCur&&d===22)e.classList.add('today');else if(isP)e.classList.add('per');else if(isO)e.classList.add('ovul');else if(goodDays.includes(d))e.classList.add('full');
    let inner=`<span>${d}</span>`;
    if((d>=19&&d<=22&&isCur)&&!isP&&!isO&&d!==22)inner+=`<div class="dtr"><div class="d4" style="background:var(--sky2)"></div><div class="d4" style="background:var(--cor)"></div><div class="d4" style="background:var(--gro)"></div><div class="d4" style="background:var(--rose)"></div></div>`;
...
function savePeriod(){const s=document.getElementById('per-start')?.value;const days=parseInt(document.getElementById('per-days')?.value)||5;if(s)S.periods.push({start:s,days});closeMod('mod-period');renderDash();}
function renderHistBars(){const c=document.getElementById('hist-bars');if(!c)return;const data=[42,58,71,45,80,65,90,55,78,82,69,75,88,75];c.innerHTML='';data.forEach((v,i)=>{const b=document.createElement('div');const isT=i===data.length-1&&S.calMon.y===2026&&S.calMon.m===3;b.style.cssText=`flex:1;border-radius:4px 4px 0 0;height:${Math.max(v*.8,4)}px;background:${isT?'linear-gradient(180deg,var(--sky2),var(--lav2))':v>=80?'rgba(91,184,245,.5)':v>=60?'rgba(179,157,219,.4)':'rgba(200,200,220,.3)'};transition:height .5s`;c.appendChild(b);});const dd=document.getElementById('hist-dates');const{y,m}=S.calMon;if(dd)dd.innerHTML=`<span>${m+1}/9</span><span>${m+1}/15</span><span>${m+1}/22</span>`;}
