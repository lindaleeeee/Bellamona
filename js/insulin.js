...
function getCycleInfo() {
  try {
    ...
    if (!p) return { id: 'unknown', name: '알수없음', day: 0 };
    const start = new Date(p.start);
    start.setHours(0, 0, 0, 0);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const totalDays = Math.floor((today - start) / 86400000);
    if (totalDays < 0) return { id: 'unknown', name: '알수없음', day: 0 };
    const dayIdx = totalDays % S.cycleLen;
    const dayNum = dayIdx + 1;

    if (dayIdx < p.days) return { id: 'menstrual', name: '생리기', day: dayNum };
    if (dayIdx >= S.cycleLen - 18 && dayIdx <= S.cycleLen - 10) return { id: 'ovulation', name: '배란기', day: dayNum };
    if (dayIdx >= S.cycleLen - 9) return { id: 'luteal', name: '황체기', day: dayNum };
    return { id: 'follicular', name: '난포기', day: dayNum };
  } catch (e) { return { id: 'unknown', name: '알수없음', day: 0 }; }
}

function currentCyclePhase() {
  return getCycleInfo().id;
}
...
function confirmFoods() { if (!S.pending.length) { closeMod('mod-food'); return; } const m = S.meals.find(x => x.id === S.activeMealId); if (m) S.pending.forEach(f => m.foods.push({ ...f })); S.pending = []; closeMod('mod-food'); goH('insulin'); if (typeof apiSaveMeal === 'function') apiSaveMeal(); }
function addMeal() { const labels = ['간식', '야식']; const used = S.meals.map(m => m.label); const label = labels.find(l => !used.includes(l)) || '간식'; const now = new Date(); const t = `${now.getHours()}:${now.getMinutes() < 10 ? '0' : ''}${now.getMinutes()}`; S.meals.push({ id: S.nextMealId++, label, time: t, foods: [], bgPre: null, bg1h: null, bg2h: null }); goH('insulin'); if (typeof apiSaveMeal === 'function') apiSaveMeal(); }
function delMeal(id) { const i = S.meals.findIndex(m => m.id === id); if (i >= 0) S.meals.splice(i, 1); goH('insulin'); if (typeof apiSaveMeal === 'function') apiSaveMeal(); }
function rmFood(mealId, fi) { const m = S.meals.find(x => x.id === mealId); if (m) { m.foods.splice(fi, 1); goH('insulin'); if (typeof apiSaveMeal === 'function') apiSaveMeal(); } }
function updQty(mealId, fi, val) { const m = S.meals.find(x => x.id === mealId); if (!m) return; const q = parseFloat(val) || 0; m.foods[fi].qty = q; m.foods[fi].g = q * m.foods[fi].food.ug; goH('insulin'); if (typeof apiSaveMeal === 'function') apiSaveMeal(); }
