...
function saveDiary(){S.checks.o4=true;const txt=document.getElementById('diary-txt')?.value;if(txt)S.diaries.push({d:'오늘',t:txt});closeMod('mod-diary');goH('oxytocin');updateHome();}
...
// ── CORTISOL ──
function renderCortisol(){return`
  <div class="card" style="border-color:rgba(206,147,216,.3)">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px">
      ...
      <div style="background:var(--cor-bg);border-radius:12px;padding:10px;text-align:center;border:1px solid rgba(206,147,216,.2)"><div style="font-size:18px;font-weight:800;color:var(--cor)">황체기</div><div style="font-size:9px;color:var(--muted);font-weight:600">21일차</div></div>
    </div>
    <div style="margin-top:8px;font-size:12px;color:var(--cor);background:var(--cor-bg);padding:8px 12px;border-radius:11px;line-height:1.5;font-weight:600">🌿 황체기 중 코르티솔이 상승하기 쉬워요. 명상과 규칙적인 수면이 핵심입니다.</div>
  </div>
  <div class="cap" style="padding:0 2px;margin-bottom:6px">루틴 체크</div>
  ${S.routines.cortisol.map(r=>ckHtml(r.id,r.label,r.sub,'var(--cor)')).join('')}`;}

// ── OXYTOCIN ──
function renderOxytocin(){return`
